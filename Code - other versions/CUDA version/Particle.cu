...
__device__ double W_poly6(const Vector3d &r, double h)
{
...
__device__ Vector3d W_poly6_grad(const Vector3d &r, double h)
{
	double r2 = r.x * r.x + r.y * r.y + r.z * r.z;
	if (r2 > h * h)
...
__device__ double W_poly6_Laplacian_modified(const Vector3d &r, double h)
{
	double r2 = r.x * r.x + r.y * r.y + r.z * r.z;
	if (r2 > h * h)
		return 0.0;
...
__device__ Vector3d W_spiky_grad(const Vector3d &r, double h)
{
...
__device__ double W_viscosity_Laplacian(const Vector3d &r, double h)
{
	double r_length = length(r);
	if (r_length > h)
		return 0.0;
...
__device__ __constant__ int neighbor_cells[27][3] =
{
	{ -1, -1, -1 },
	{ -1, -1, 0 },
	{ -1, -1, 1 },
...
__device__ void Particle::update_acceleration(int x, int y, int z, Particle_node* cells[CELL_NUM_X][CELL_NUM_Y][CELL_NUM_Z])
{
...
__device__ void Particle::update_velocity()
{
	velocity = velocity + acceleration * (DELTA_T / 2);
}

__device__ void Particle::update_position()
{
	position = position + velocity * DELTA_T;
	handleBoundary();
}

__device__ void Particle::calc_density_and_pressure(int x, int y, int z, Particle_node* cells[CELL_NUM_X][CELL_NUM_Y][CELL_NUM_Z])
{
	double sum = 0.0;
	for (int ii = 0; ii < 27; ++ii)
	{
...
__device__ void Particle::handleBoundary() {
	if (position.x < 0) {
		position.x = 0;
